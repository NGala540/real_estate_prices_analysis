...
The slope of the read line (smoothing) is the greatest for the cities with the most expensive apartments, so the result is correlated.
...
Amenities (all but storageRoom) seem to have a noticeable positive impact on the price of the apartment.
...
There is no strong correlation found between the level of prices and distance to public places. However there is pretty solid positive correlation between variables of distances to public places. !!! Weak correlation for clinicDistance and restaurantDistance seemingly points to the fact that the closer the clinic, the more expensive the appartment.
...
print(dfSummary(data_buy, na.col=FALSE, style="multiline", 
                plain.ascii=FALSE, headings = FALSE, graph.magnif = .8),
      method = "render"
)
```
...
2) The most common building type is block of flats, while apartment buildings and tenements occupy ~20% of the market each;
3) Average housing area (m²) is 58.7, while this variable has right-skewed distribution;
4) The most widespread are 2 and 3 room apartments, whereas other categories make up under 20% of the total;
...
8) Over 90% of apartments are owned on condominium agreement.

```{r}
print(dfSummary(data_rent, na.col=FALSE, style="multiline", 
                plain.ascii=FALSE, headings = FALSE, graph.magnif = .8),
      method = "render"
)
```
Rental market data in general follow the same trend, however there are some differences:
2) Apartment buildings are the most common type of building the housing is located in;
3) Area of the apartments is a bit lower (average 55.3 m²) than in case of purchase market;
4) Two room apartments are the most represented category with over 55% of items;
7) The average of build year is higher for rental market (by 12 years), which is explained by higher attractiveness of modern buildings for tenants;
19) Almost each apartment is declared to be in premium condition.


### Share of market by city with contingency tables

```{r contingency_table1}
data_buy <- complete_buy
...
It follows from the data that the largest share of the apartments market is in Warsaw (above 30% of the country market), smaller parts of the market represent next major cities: Kraków (14.84%), Wrocaław (10.32%) and Gdańsk (9.82%).

```{r contingency_table1.1}
data_rent <- complete_rent
...
Warsaw has even larger part of the rent market over 40%, growth also noticeable in the market share of Krakow (+3%). Remaining cities have minor shares of this market (under 10%).

### Price distribution shape with tabular data

```{r contingency_table2}
...
hist(data_buy$price, prob=TRUE, breaks=limits, main="Price groupping for purchase market", xlab="Price in PLN", col="lightblue")
...
plot(tab_accuracy, pal=pcol, ylab='Cumulative %')
...
hist(data_rent$price, prob=TRUE, breaks=limits, main="Price groupping for rental market", xlab="Price in PLN", col="lightblue")
...
```
The shapes of the rental market distribution is narrower than purchase market one, which can be explained by the target groups of these offers.

```{r}
pl1 <- data_buy %>%
  ggplot(aes(x=price_per_m2)) +
  geom_density(color="#69b3a2", alpha=0.8) +
  geom_vline(aes(xintercept=median(price_per_m2)), linetype="dashed", size=0.5, color="69b3a2") +
  annotate("text", x=12000, y=0.00004, label=paste("Median: ", as.character(median(round(data_buy$price_per_m2, 2)))), angle=90, color="69b3a2") +
...
pl2 <- data_rent %>%
  ...
  geom_density(color="#69b3a2", alpha=0.8) +
  geom_vline(aes(xintercept=median(price_per_m2)), linetype="dashed", size=0.5, color="69b3a2") +
  annotate("text", x=60, y=0.01, label=paste("Median: ", as.character(median(round(data_rent$price_per_m2, 2)))), angle=90, color="69b3a2") +
...
grid.arrange(pl1+theme(legend.position="top"),
    ...
    ncol = 2,
    top =textGrob("Price per square meter",gp=gpar(fontsize=20,font=3)))
```
Prices per square meter are more similar to the normal distribution.
...
```{r}
# statistical significance of price median differance between cities
...
Based on Welch's test, we obtained a p-value close to 0, so we reject the null hypothesis of equal means between populations in both the buy and rent datasets. This means that the city statistically significantly differentiates both rent and buy prices. However, an effect size of 0.86 implies that this differentiation has a moderate magnitude..
...
Finally, condition of flat in both rent and buy dataset also seem to differenciate price significantly but has only moderate magnitude.
```
