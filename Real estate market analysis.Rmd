...
For finding outliers we used couple methods: - grubbs test - boxplots - Interqunatile Range Method - Q-Q plot
...
Data validation ensures data correctness by verifying logical consistency and identifying anomalies. In this data set, we checked the following properties: - Floor number is less than or equal to the total floor count. - Build year is less than 2024. - Latitude and longitude fall within Poland's geographic range.
...
# complete_buy <- read.csv("clean_buy.csv")
# complete_buy <- complete_buy %>% 
#   select(-X)
# complete_rent <- read.csv("clean_rent.csv")
# complete_rent <- complete_rent %>% 
#   select(-X)
# or
complete_buy <- complete_data_buy
complete_rent <- complete_data_rent
```
...
3)  Area of the apartments is a bit lower (average 53.4 m²) than in case of purchase market;
4)  Two room apartments are the most represented category with over 57% of items;
5)  The average of build year is higher for rental market (by 12 years), which is explained by higher attractiveness of modern buildings for tenants;
6)  97.7% of the apartments is declared to be in premium condition.
...
Warsaw has even larger part of the rent market over 40%, growth also noticeable in the market share of Krakow (+4%). Remaining cities have minor shares of this market (under 10% each).
